...
WITH Topics AS
  (SELECT Topic.id as topic, title, Forum.name as name, Forum.id as forumID FROM Topic
  JOIN Forum ON Forum.id = forumID
  WHERE forumID = 4)
  SELECT count(1) as c, topic, title, Topics.name FROM Forum
  INNER JOIN Topics ON Forum.id = Topics.forumID;
...
WITH Count AS
  ...
  JOIN Forum ON Forum.id = forumID
  WHERE forumID = 4)
  SELECT Count.c, Count.name, Topic.id, title FROM Topic
